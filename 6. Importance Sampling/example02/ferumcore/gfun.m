...
ABAQUSinput;
   if grad_flag == 'no '
      [ G, dummy ] = FERUMABAQUS(lsf,x,'no ',gfundata,femodel,randomfield,ABAQUSinput);
      grad_g = 0;
   elseif grad_flag == 'ffd'
      parameter = probdata.parameter;
      %marg = probdata.marg;
      ...
      for j = 1 : length(x)
         ...
         perturbed_x = x;
         perturbed_x(j) = perturbed_x(j) + h;
         [ G_a_step_ahead, dummy ] = FERUMABAQUS(lsf,perturbed_x,'no ',gfundata,femodel,randomfield,ABAQUSinput);
         grad_g(j) = (G_a_step_ahead - G)/h;
      end
   elseif grad_flag == 'ddm'
      [ G, grad_g ] = FERUMABAQUS(lsf,x,'yes',gfundata,femodel,randomfield,ABAQUSinput);
   else
      disp('ERROR: Invalid method for gradient computations');
   end
   
case 'b3_method_ferumfatigue'
   if grad_flag == 'no '
      [ G, dummy ] = B3_method_FERUMfatigue(lsf,x,'no ',gfundata,femodel,randomfield);
      grad_g = 0;
   elseif grad_flag == 'ffd'
      parameter = probdata.parameter;
      %marg = probdata.marg;
      ...
      for j = 1 : length(x)
         ...
         perturbed_x = x;
         perturbed_x(j) = perturbed_x(j) + h;
         [ G_a_step_ahead, dummy ] = B3_method_FERUMfatigue(lsf,perturbed_x,'no ',gfundata,femodel,randomfield);
         grad_g(j) = (G_a_step_ahead - G)/h;
      end
   elseif grad_flag == 'ddm'
      [ G, grad_g ] = B3_method_FERUMfatigue(lsf,x,'yes',gfundata,femodel,randomfield);
      G
      grad_g
   else
      disp('ERROR: Invalid method for gradient computations');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% modified by Nophi2025

case 'g_fun'
   thetag = gfundata(lsf).thetag;
   [G, dummy] = g_fun(x,thetag);
   if grad_flag == 'no '
       grad_g = 0;
   elseif grad_flag == 'ffd'
       parameter = probdata.parameter;
       ...
       for j = 1 : length(x)
           ...
           x = original_x;
           x(j) = x(j) + h;
           [G_a_step_ahead, dummy] = g_fun(x,thetag);
           grad_g(j) = (G_a_step_ahead - G)/h;
       end
   elseif grad_flag == 'ddm'
       [dummy, grad_g] = g_fun(x,thetag);
   else
       disp('ERROR: Invalid method for gradient computations');
   end


case 'bcube_limit_state_orig'
   if grad_flag == 'no '
      [ G, dummy ] = bcube_limit_state_orig(lsf,x,'no ',gfundata,femodel,randomfield);
      grad_g = 0;
   elseif grad_flag == 'ffd'
      parameter = probdata.parameter;
      %marg = probdata.marg;
      ...
      for j = 1 : length(x)
         ...
         perturbed_x = x;
         perturbed_x(j) = perturbed_x(j) + h;
         [ G_a_step_ahead, dummy ] = bcube_limit_state_orig(lsf,perturbed_x,'no ',gfundata,femodel,randomfield);
         grad_g(j) = (G_a_step_ahead - G)/h;
      end
   elseif grad_flag == 'ddm'
      [ G, grad_g ] = bcube_limit_state_orig(lsf,x,'yes',gfundata,femodel,randomfield);
   else
      disp('ERROR: Invalid method for gradient computations');
   end


case 'bcube_limit_state_orig_improved'
   if grad_flag == 'no '
      [ G, dummy ] = bcube_limit_state_orig_improved(lsf,x,'no ',gfundata,femodel,randomfield);
      grad_g = 0;
   elseif grad_flag == 'ffd'
      parameter = probdata.parameter;
      %marg = probdata.marg;
      ...
      for j = 1 : length(x)
         ...
         perturbed_x = x;
         perturbed_x(j) = perturbed_x(j) + h;
         [ G_a_step_ahead, dummy ] = bcube_limit_state_orig_improved(lsf,perturbed_x,'no ',gfundata,femodel,randomfield);
         grad_g(j) = (G_a_step_ahead - G)/h;
      end
   elseif grad_flag == 'ddm'
      [ G, grad_g ] = bcube_limit_state_orig_improved(lsf,x,'yes',gfundata,femodel,randomfield);
   else
      disp('ERROR: Invalid method for gradient computations');
   end   

case 'fatigue_limit_state'
   if grad_flag == 'no '
      [ G, dummy ] = fatigue_limit_state(lsf,x,'no ',gfundata,femodel,randomfield);
      grad_g = 0;
   elseif grad_flag == 'ffd'
      parameter = probdata.parameter;
      %marg = probdata.marg;
      ...
      for j = 1 : length(x)
         ...
         perturbed_x = x;
         perturbed_x(j) = perturbed_x(j) + h;
         [ G_a_step_ahead, dummy ] = fatigue_limit_state(lsf,perturbed_x,'no ',gfundata,femodel,randomfield);
         grad_g(j) = (G_a_step_ahead - G)/h;
      end
   elseif grad_flag == 'ddm'
      [ G, grad_g ] = fatigue_limit_state(lsf,x,'yes',gfundata,femodel,randomfield);
   else
      disp('ERROR: Invalid method for gradient computations');
   end      
